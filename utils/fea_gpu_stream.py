...
def _kinetic_feature(single_sample):
    single_sample=torch.from_numpy(single_sample).cuda()
    pt_num = 0.95
...
def parallel_execution(func,samples):
    streams = []
    results = []
...
def kinetic_feature(sample_list,n_jobs:int = 1):
    if n_jobs == 1: 
        #sample_list = [torch.from_numpy(single_sample).cuda() for single_sample in sample_list]
        ...
        #sample_list = [torch.from_numpy(single_sample).cuda() for single_sample in sample_list]
        feature_list = parallel_execution(_kinetic_feature,sample_list)
        features = torch.stack(feature_list)
        features =  features.numpy()
        
         # 找到无穷大的位置
        inf_indices = np.isinf(features)

        # 将无穷大的位置置零
        features[inf_indices] = 0
        
        # 找到NaN的位置
        inf_indices = np.isnan(features)
